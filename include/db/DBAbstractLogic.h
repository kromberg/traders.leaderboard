...
public:
    ...
    virtual Result onUserDeal(const uint64_t id, const ptime t, const uint64_t amount) = 0;
    // user_deal_won(id,time,amount)
    virtual Result onUserDealWon(const uint64_t id, const ptime t, const uint64_t amount) = 0;
    // user_connected(id)
